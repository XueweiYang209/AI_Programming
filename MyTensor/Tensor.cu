...
void Tensor::allocateMemory() {
    if (device_ == Device::CPU) {
        ...
        float *gpuPtr;
        CUDA_CHECK(cudaMalloc(&gpuPtr, total_size_ * sizeof(float)));
        initKernel<<<CudaGetBlocks(total_size_), kCudaThreadsNum>>>(
            gpuPtr, total_size_);
        cudaDeviceSynchronize();
        value = std::shared_ptr<float[]>(gpuPtr, [](float *ptr) {
...
Tensor Tensor::cpu() {
    ...
    if (this->device_ == Device::CPU) {
        memcpy(t.value.get(), this->value.get(),
               total_size_ * sizeof(float)); // 深拷贝
    } else {
        CUDA_CHECK(cudaMemcpy(t.value.get(), this->value.get(),
